...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(playerped, false) and not LocalPlayer.state.disableinput then
                ...
                if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
                    ...
                    if not IsPauseMenuActive() then
                        ...
                        elseif IsDisabledControlJustReleased(0, 83) then -- INPUT_VEH_NEXT_RADIO_TRACK
                            ...
                            count_bcast_timer = delay_bcast_timer
                        -- IND H
                        elseif IsControlJustReleased(0, 194) then -- INPUT_FRONTEND_CANCEL / Backspace
                    ...
                    if count_bcast_timer > delay_bcast_timer then
                        ...
                        --- IS EMERG VEHICLE ---
                        if GetVehicleClass(veh) == 18 then
                            local emergency = GetClosePlayers(GetEntityCoords(PlayerPedId()), 300.0)
                            TriggerServerEvent("lvc_TogDfltSrnMuted_s", dsrn_mute, emergency)
                            TriggerServerEvent("lvc_SetLxSirenState_s", state_lxsiren[veh], emergency)
                            -- TriggerServerEvent("lvc_TogPwrcallState_s", state_pwrcall[veh], emergency)
                            TriggerServerEvent("lvc_SetAirManuState_s", state_airmanu[veh], emergency)
                        end
                        --- IS ANY OTHER VEHICLE ---
                        TriggerServerEvent("lvc_TogIndicState_s", state_indic[veh], GetClosePlayers(GetEntityCoords(PlayerPedId()), 80.0))
                    else
