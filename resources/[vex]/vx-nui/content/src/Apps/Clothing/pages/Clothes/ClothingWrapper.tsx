...
import { ClothingComponent } from "./clothes";

interface ClothingWrapperProps {
  data: ClothingComponent;
}

export interface ClothingComponentData {
  drawableId: number;
  textureId: number;
  drawableMax: number;
  textureMax: number;
...
const ClothingWrapper: React.FunctionComponent<ClothingWrapperProps> = ({
  data,
}) => {
  ...
  React.useEffect(() => {
    const asyncWrapper = async () => {
      ...
      const res = await post<any>("/clothing/clothes/component/current", {
  ...
  const onNextDrawable = async (value: number) => {
    ...
    if (newDrawableId >= maxDrawable) {
      newDrawableId = 0;
    } else if (newDrawableId <= 0) {
      newDrawableId = maxDrawable - 1;
    }

    await fetcher("/clothing/clothes/component/drawable/set", {
      componentId: data.componentId,
